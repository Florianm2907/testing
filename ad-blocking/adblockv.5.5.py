...
def audio(vol):
    ...
    for session in sessions:
        ...
        if session.Process and session.Process.name() == "adblockv.5.5.exe" or session.Process and session.Process.name() == "python.exe":
...
while True:
    ...
    if "Advertisement" in titles:
        ...
        if not play:
            winsound.PlaySound(r'./wartemusik.wav', winsound.SND_ASYNC)
